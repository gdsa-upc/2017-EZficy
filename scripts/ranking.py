...
def rank(params):
    ...
    for val_id in val_features.keys():
        ...
        # Save to text file
        outfile = open(os.path.join(params['root'],params['root_save'],params['rankings_dir'],params['descriptor_type'],params['split'],val_id.split('/')[-1].split('.')[0] + '.txt'),'w')
        
        for item in ranking:

            outfile.write(item.split('/')[-1].split('.')[0] + '\n')
...
if __name__ == "__main__":
    print("Starting...")
    params = get_params()
    print("Ranking feature distances for each image...")
    rank(params)
    print("Done!")